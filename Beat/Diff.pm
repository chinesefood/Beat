    ...
    while (!$of->eof() && !$nf->eof()) {
        ...
        for (my $i = 0; $i <= $length; $i++) {
            ...
            if ($matched && !$in_delta) {
                ...
                push @records, $self->_make_rle_records({
                    'offset'    => $offset,
                    'data'      => $delta,
                });
            }
...

{
    sub _make_rle_records {
        ...
        my $o = $args_ref->{'offset'};
        my $d = $args_ref->{'data'};
        my @records;
        
        # In order for RLE records to be worth the trouble, the data must be
        # longer than 8 bytes, or else no compression is done.
        ...
        for (my $i = 0; $i < length $d; $i++) {
            ...
            if ($c eq substr $d, $i + 1, 1) {
            }
            else {
                push @records, Beat::Record::RLE->new({
                    'data'      => $ns,
                    'offset'    => $o,
                });
